...
mkdir -p "checkpoints/4-1/${GRAMMAR}/${SPLIT}-lstm"
mkdir -p "lstm-results/4-1"
mkdir -p "sentence_scores_lstm/4-1"
...
if [ -f "checkpoints/4-1/${GRAMMAR}/${SPLIT}-lstm/checkpoint_best.pt" ]; then
    TRAIN_CMD="$TRAIN_CMD --restore-file \"checkpoints/4-1/${GRAMMAR}/${SPLIT}-lstm/checkpoint_best.pt\""
fi
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "valid" \
    --output-word-probs \
    --quiet 2> "lstm-results/4-1/${GRAMMAR}.${SPLIT}.dev.txt"
