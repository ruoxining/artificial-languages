...
if [ -f "checkpoints/base/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt" ]; then
    TRAIN_CMD="$TRAIN_CMD --restore-file \"checkpoints/base/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt\""
fi
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "valid" \
    --output-word-probs \
    --quiet 2> "transformer-results/base/${GRAMMAR}.${SPLIT}.dev.txt"
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "test" \
    --output-word-probs \
    --quiet 2> "transformer-results/base/${GRAMMAR}.${SPLIT}.test.txt"

python get_sentence_scores.py -i "transformer-results/base/${GRAMMAR}.${SPLIT}.test.txt" -O "sentence_scores_transformer/base/" 
