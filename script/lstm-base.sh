...
mkdir -p "checkpoints/base/${GRAMMAR}/${SPLIT}-lstm"
mkdir -p "lstm-results/base"
mkdir -p "sentence_scores_lstm/base"
...
if [ -f "checkpoints/base/${GRAMMAR}/${SPLIT}-lstm/checkpoint_best.pt" ]; then
    TRAIN_CMD="$TRAIN_CMD --restore-file \"checkpoints/base/${GRAMMAR}/${SPLIT}-lstm/checkpoint_best.pt\""
fi
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "valid" \
    --output-word-probs \
    --quiet 2> "lstm-results/base/${GRAMMAR}.${SPLIT}.dev.txt"
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "test" \
    --output-word-probs \
    --quiet 2> "lstm-results/base/${GRAMMAR}.${SPLIT}.test.txt"

python get_sentence_scores.py -i "lstm-results/base/${GRAMMAR}.${SPLIT}.test.txt" -O "sentence_scores_lstm/base/"
