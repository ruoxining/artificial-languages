...
TRAIN_CMD="fairseq-train --task language_modeling \"data-bin/base/${GRAMMAR}/${SPLIT}-dataset\" \
    ...
    --decoder-input-dim 256 \
    --decoder-output-dim 256 \
...
if [ -f "checkpoints/8-1/${GRAMMAR}/${SPLIT}-transformer/checkpoint_last.pt" ]; then
    TRAIN_CMD="$TRAIN_CMD --restore-file \"checkpoints/8-1/${GRAMMAR}/${SPLIT}-transformer/checkpoint_last.pt\""
fi
