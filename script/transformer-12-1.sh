...
mkdir -p "checkpoints/12-1/${GRAMMAR}/${SPLIT}-transformer"
mkdir -p "transformer-results/12-1"
mkdir -p "sentence_scores_transformer/12-1"
...
if [ -f "checkpoints/12-1/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt" ]; then
    TRAIN_CMD="$TRAIN_CMD --restore-file \"checkpoints/12-1/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt\""
fi
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "valid" \
    --output-word-probs \
    --quiet 2> "transformer-results/12-1/${GRAMMAR}.${SPLIT}.dev.txt"
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "test" \
    --output-word-probs \
    --quiet 2> "transformer-results/12-1/${GRAMMAR}.${SPLIT}.test.txt"

python get_sentence_scores.py -i "transformer-results/12-1/${GRAMMAR}.${SPLIT}.test.txt" -O "sentence_scores_transformer/12-1/" 
