...
mkdir -p "checkpoints/3-2/${GRAMMAR}/${SPLIT}-lstm"
mkdir -p "lstm-results/3-2"
mkdir -p "sentence_scores_lstm/3-2"
...
if [ -f "checkpoints/3-2/${GRAMMAR}/${SPLIT}-lstm/checkpoint_best.pt" ]; then
    TRAIN_CMD="$TRAIN_CMD --restore-file \"checkpoints/3-2/${GRAMMAR}/${SPLIT}-lstm/checkpoint_best.pt\""
fi
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "valid" \
    --output-word-probs \
    --quiet 2> "lstm-results/3-2/${GRAMMAR}.${SPLIT}.dev.txt"
...
fairseq-eval-lm "data-bin/base/${GRAMMAR}/${SPLIT}-dataset" \
    ...
    --gen-subset "test" \
    --output-word-probs \
    --quiet 2> "lstm-results/3-2/${GRAMMAR}.${SPLIT}.test.txt"

python get_sentence_scores.py -i "lstm-results/3-2/${GRAMMAR}.${SPLIT}.test.txt" -O "sentence_scores_lstm/3-2/"
