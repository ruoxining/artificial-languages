...
mkdir -p "checkpoints/8-2/${GRAMMAR}/${SPLIT}-transformer"
mkdir -p "transformer-results/8-2"
mkdir -p "sentence_scores_transformer/8-2"
...
TRAIN_CMD="fairseq-train --task language_modeling \"data-bin/base/${GRAMMAR}/${SPLIT}-dataset\" \
    --save-dir \"checkpoints/8-2/${GRAMMAR}/${SPLIT}-transformer\" \
    --arch transformer_lm \
...
if [ -f "checkpoints/8-2/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt" ]; then
    TRAIN_CMD="$TRAIN_CMD --restore-file \"checkpoints/8-2/${GRAMMAR}/${SPLIT}-transformer/checkpoint_best.pt\""
fi
...
python get_sentence_scores.py -i "transformer-results/8-2/${GRAMMAR}.${SPLIT}.test.txt" -O "sentence_scores_transformer/8-2/" 
